...
class ExchangeRateListVC: UITableViewController, CustomDateFormatter {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if self.appDelegate.onExchangeRateUpdate == true {
            self.appDelegate.onExchangeRateUpdate = false
            self.updateData(isRefresh: false)
        }
    }
    ...
    override func viewDidLoad() {
        // 이벤트 기록
        Analytics.logEvent("환율_메인_화면", parameters: ["환율_메인_화면": "환율_메인_화면" as NSObject])
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        guard let vc = self.storyboard?.instantiateViewController(withIdentifier: "ExchangeRateChartView") as? ExchangeRateChartVC else { return }
