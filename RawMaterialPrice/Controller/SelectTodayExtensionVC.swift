...
class SelectTodayExtensionVC: UITableViewController {
    ...
    // MARK: - Property
    private let listDAO = TodayExtensionListDAO()
    
    // 선택된 항목
    lazy var selectionList: [TodayExtensionListVO] = {
        return listDAO.findList(selection: true)
    }()
    
    // 선택되지 않은 항목
    lazy var nonSelectionList: [TodayExtensionListVO] = {
        return listDAO.findList(selection: false)
    }()
    ...
    override func viewDidLoad() {
        // 이벤트 기록
        Analytics.logEvent("위젯추가_화면", parameters: ["위젯추가_화면": "위젯추가_화면" as NSObject])
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        ...
        if indexPath.section == 0 {
            ...
            guard listDAO.editTurn(tableName: self.selectionList[indexPath.row].tableName!, turn: Int32(41)) == true else {
            ...
            if self.nonSelectionList.count == 0 {
                ...
                for row in 0..<self.nonSelectionList.count {
                    if self.selectionList[indexPath.row].cd! < self.nonSelectionList[row].cd! {
        ...
        // 리스트 갱신
        self.selectionList = listDAO.findList(selection: true)
        self.nonSelectionList = listDAO.findList(selection: false)
